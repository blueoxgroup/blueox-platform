...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "image/*", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for client-documents" ON storage.objects FOR SELECT USING (bucket_id = 'client-documents');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for client-documents" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'client-documents');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for client-documents" ON storage.objects FOR UPDATE USING (bucket_id = 'client-documents');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for client-documents" ON storage.objects FOR DELETE USING (bucket_id = 'client-documents');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'client-documents',
        public: true,
        allowed_mime_types: ["application/pdf", "image/*", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"],
